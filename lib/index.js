...
Publisher.prototype.sync = function(options) {
  ...
  stream._flush = function(cb) {
    ...
    lister = client.listObjects({ Prefix: options.prefix })
      .createReadStream()
      .pipe(converter('Key'));
    ...
    lister.on('data', function (key) {
      ...
      // is included in the published files, don't delete it
      if (newFiles[key]) {
        return;
      }

      // matches the exclude pattern, don't delete it
      if (options.exclude && key.match(options.exclude)) {
        return;
      }
